...
let current = computed(()=>props.context?.page)
let pageSize = computed(()=>props.context?.pageSize)
let total = computed(()=>props.context?.total)

let emit = defineEmits(['update:current'])

let totalPage = computed(()=>Math.ceil(total.value / pageSize.value))

const distance = 2
let range = computed(()=>{
    let _current = current.value
    let _totalPage = totalPage.value
    let pivot = Math.max(3, _current)
